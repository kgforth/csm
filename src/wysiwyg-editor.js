                                  ...
                                  placeholder_url, max_imagesize, onImageUpload, onKeyEnter )
    {
        ...
        var content_colorpalette = function( wysiwygeditor, forecolor )
        {
            var $content = $('<table/>')
                            ...
                            .attr('cellspacing','0')
                            .attr('unselectable','on');
            for( var row=1; row < 2; ++row ) // should be '16' - but last line looks so dark
            {
                var $rows = $('<tr/>');
                for( var col=0; col < 6; ++col ) // last column is grayscale
                {
                    var color;
//                    if( col == 7 )
//                    {
//                        var gray = Math.floor(255 / 3 * (4 - row)).toString(16);
...
//                    {
                        var hue        = col / 6;
                        var saturation = row <= 6 ? row     /1 : 1;
                        var value      = row  > 1 ? (2-row)/6 : 1;
                        color = HSVtoRGB( hue, saturation, value );

//      change yellow color to dark yellow
